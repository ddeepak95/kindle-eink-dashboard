#!/bin/sh
# Loaded by the local render-layout.sh worker.
...
if [ "$SHOW_QUOTE" = "1" ]; then
    quote_source="$DASHBOARD_LAYOUT_DIR/quotes.txt"
...
header=$(date '+%A, %B %d')
icons="$DASHBOARD_LAYOUT_DIR/icons"
...
render_truetype() {
    ...
    text_box 32 477 551 52 700 BOLD "NEXT $HOURLY_FORECAST_COUNT HOURS"
    i=0
    if [ "$group_count" -gt 0 ]; then
        printf '%s\n' "$groups" | while IFS='|' read -r first last low high hour_code hour_rain; do
            ...
            if [ "$group_count" -gt 6 ]; then
                ...
                top=$((520 + (i / columns) * 150))
                text_box 32 "$top" $((1072 - top - 38)) "$left" "$right" BOLD "$(period_label "$first" "$last")"
                draw_icon "$hour_code" "$left" $((top + 38)) 40 || true
                temperatures="$low"; [ "$low" = "$high" ] || temperatures="$low-$high"
                text_box 36 $((top + 38)) $((1072 - top - 80)) $((left + 46)) "$right" BOLD "$temperatures"
                text_box 27 $((top + 80)) $((1072 - top - 112)) "$left" "$right" BOLD "$(weather_description "$hour_code")"
                text_box 32 $((top + 112)) $((1072 - top - 150)) "$left" "$right" BOLD "Rain $hour_rain%"
                [ "$hour_code" -lt 51 ] || draw_rule $((top + 148)) "$left" $((width - 20)) 3
                i=$((i + 1))
                continue
            fi
            left=$((52 + i * width)); right=$((1448 - left - width + 20)); icon_x=$((left + width / 2 - 40))
            text_box 34 529 495 "$left" "$right" BOLD "$(period_label "$first" "$last")"
            draw_icon "$hour_code" "$icon_x" 584 80 || true
            temperatures="$low"; [ "$low" = "$high" ] || temperatures="$low-$high"
            text_box 46 674 332 "$left" "$right" BOLD "$temperatures"
            text_box 29 731 270 "$left" "$right" BOLD "$(weather_description "$hour_code")"
            text_box 36 776 251 "$left" "$right" BOLD "Rain $hour_rain%"
